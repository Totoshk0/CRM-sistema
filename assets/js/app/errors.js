const getError = (input = null, errorReason, id = "errorsContainer_ID") => {
  ...
  setTimeout(() => {
    errorsContainer.style.height =
      +errorsContainer.style.height.replace("px", "") +
      error.clientHeight +
      "px";
  ...
  if (input) {
    input.type === "text"
      ? (inputLabel = input.nextSibling.innerText)
      : input.type === "tel" || input.type === "email"
      ? (inputLabel = input.previousSibling.querySelector(
          ".contact__select-head"
        ).innerText)
      : input.type === "url"
      ? (inputLabel = "Профиль " + input.dataset.url + " ")
      : false;
