...
function getModalContent(modalTitle, { onOpen, onClose }, value) {
  ...
  const [surnameContainer, nameContainer, lastNameContainer] = Array(3)
    .fill("")
    .map((container) => {
  ...
  const [surnameInput, nameInput, lastNameInput] = Array(3)
    .fill("")
    .map((input) => {
  ...
  const [surnameLabel, nameLabel, lastNameLabel] = Array(3)
    .fill("")
    .map((label) => {
  ...
  replaceAddContactButton.classList.add(
    "replace-modal-add-contact-button",
    "add-contact-button__hidden"
  );
  ...
  replaceAddContactButton.innerText =
    "Достигнуто максимальное количество контактных данных";
  ...
  if (value) {
    ...
    getFormValidation(form, title, value, modalContainer, modalWindow, {
      onClose,
    });
  } else
    getFormValidation(form, title, null, modalContainer, modalWindow, {
      onClose,
    });
  ...
  inputs.forEach((input) => {
    input.value.length
      ? input.nextElementSibling.classList.add("input-label-transform")
      : false;

    input.onblur = () =>
      !input.value.length
        ? input.nextElementSibling.classList.remove("input-label-transform")
        : false;
...
function getNewContact(
  ...
  const [container, select, selectHead, selectBody] = Array(4)
    .fill("")
  ...
  const checkedSelectBody = () =>
    selectBody.childNodes.forEach((option) => {
  ...
  const checkedSelectHead = () => {
    ...
    if (
      selectHead.innerText === "Телефон" ||
      selectHead.innerText === "Доп. телефон"
    ) {
    ...
    selectHead.innerText === "Email" ? (input.type = "email") : false;

    selectHead.innerText === "Vk"
      ? (input.dataset.url = "vk.com")
      : selectHead.innerText === "Facebook"
      ? (input.dataset.url = "facebook.com")
      : selectHead.innerText === "Instagram"
      ? (input.dataset.url = "instagram.com")
      : selectHead.innerText === "Twitter"
      ? (input.dataset.url = "twitter.com")
      : selectHead.innerText === "Ok"
      ? (input.dataset.url = "ok.ru")
      : selectHead.innerText === "Мой Мир"
      ? (input.dataset.url = "my.mail.ru")
      : selectHead.innerText === "LinkedIn"
      ? (input.dataset.url = "linkedin.com")
      : false;
  };
  ...
  allContactsContainer.style.height =
    +allContactsContainer.style.height.replace("px", "") +
    WIDTH_CONTACT_CONTAINER +
    "px";
  ...
  input.addEventListener("input", (e) => {
    ...
    if (input.type === "url") {
      const value = input.value
        .trim()
        .toLowerCase()
        .replace(/[^a-zа-я\-_:?%.\d/]/g, "");
      let formattedValue = value;

      input.value[0] === "/"
        ? (formattedValue = input.dataset.url + "/")
        : false;

      /\w\//.test(formattedValue)
    ...
    if (input.type === "text") {
      const value = input.value
        ? input.value[0].toUpperCase().replace(/[\-\s]/i, "") +
      ...
      input.value = formattedValue.replace(/[\s\-].?/g, (str) =>
        str.toUpperCase()
      );
    }
  });
...
function getFormValidation(
  ...
  form.addEventListener("submit", (e) => {
    ...
    input.forEach((input) => {
      ...
      if (input.type === "text") {
        input.value.length === 1
          ? getError(input, "не может содержать менее 2 символов")
          : input.value.length > 32
          ? getError(input, "не может содержать более 32 символов")
          : /^.?(-|\s).?|.?(-|\s).?$/.test(input.value)
          ? getError(
              input,
              "перед и после дефиса или пробела не может быть менее 2 символов"
            )
          : /(?=(.))\1{3,}/gi.test(input.value)
          ? getError(
              input,
              "не может содержать 3 или более повторяющихся символов подряд"
            )
          : !input.value &
            (input.id === "surnameInput_ID" || input.id === "nameInput_ID")
          ? getError(input, "это поле обязательно для заполнения")
          : false;
      }

      if (input.type === "tel") {
        input.value && input.value.match(/\d/g).length < 11
          ? getError(input, "должен содержать минимум 11 цифр")
          : /\+[0-689]/.test(input.value)
          ? getError(input, "с любым кодом страны кроме +7 не принимается")
          : !input.value
          ? getError(input, "поле не может быть пустым")
          : false;
      }
      ...
      if (input.type === "url") {
        ...
        !input.value
          ...
          ? getError(
              input,
              `некорректная ссылка на профиль (пример: ${input.dataset.url}/ваш_id)`
            )
          : false;
      }
      ...
      if (input.type === "email") {
        !input.value
          ? getError(input, "поле не может быть пустым")
          : !/[-\w]{2,}@[-\w.]{2,10}\.[\w]{2,10}/.test(input.value)
          ? getError(
              input,
              "некорректный почтовый адрес (пример: mail@mail.ru)"
            )
          : input.value.length > 256
          ? getError(input, "поле не может содержать более 256 символов")
          : false;
      }
    });
    ...
    if (errorsContainer_ID.innerHTML === "") {
      ...
      title.innerText === "Изменить данные"
        ? editClient(getClientData, value.id)
        : false;
    }
...
function getClientData() {
  const contactsData = () => {
    const contactsArr = [
      ...allContactsContainer_ID.getElementsByTagName("input"),
    ];

    return contactsArr.reduce((acc, input) => {
      const label = input.previousSibling.querySelector(
